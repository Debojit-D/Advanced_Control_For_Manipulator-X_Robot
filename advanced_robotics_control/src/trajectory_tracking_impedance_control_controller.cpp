...
#include "advanced_robotics_control/trajectory_tracking_impedance_control_controller.h"

namespace advanced_robotics_control
{
bool TrajectoryTrackingImpedanceControlController::init(
    hardware_interface::RobotHW* robot_hardware, ros::NodeHandle& node_handle)
{
  ...
  {
    ROS_ERROR(
        "[TrajectoryTrackingImpedanceControlController] Could not get effort joint interface "
        "from hardware!");
    return false;
  }

  // Initialize desired position (x_d_) and stiffness (K_trans)
  x_d_ = KDL::Vector(0.00, 0.0000, 0.2500);  // This line defines the desired postion of the end effector of the robot. 
  x_o_ = KDL::Vector(0.0000, 0.0000, 0.0000);  // This line defines the desired orientation of the end effector of the robot.
  K_trans_ = KDL::Vector(100, 100, 100);  // Stiffness in x, y, z directions
  K_rot_ = KDL::Vector(1.0, 1.0, 1.0); // Stiffness in roll, pitch and yaw
  ...
  Kd_lin_ = KDL::Vector(2, 2, 2); // Translational Damping in x,y,z direction
  ...
  {
    ROS_ERROR("[TrajectoryTrackingImpedanceControlController] Could not parse joint names");
    return false;
  }
  for (size_t i = 0; i < joint_names_.size(); i++)
  {
    ...
    {
      ROS_ERROR_STREAM(
          "[TrajectoryTrackingImpedanceControlController] Could not get joint handle: "
          << e.what());
      return false;
    }
  }
  ...
  {
    ROS_ERROR("[TrajectoryTrackingImpedanceControlController] Could not parse urdf file");
    return false;
  }
  if (!kdl_parser::treeFromUrdfModel(urdf, kdl_tree_))
  {
    ROS_ERROR("[TrajectoryTrackingImpedanceControlController] Could not construct kdl tree");
    return false;
  }
  if (!node_handle.getParam("root_link", root_name_))
  {
    ROS_ERROR("[TrajectoryTrackingImpedanceControlController] Could not find root link name");
    return false;
  }
  if (!node_handle.getParam("tip_link", tip_name_))
  {
    ROS_ERROR("[TrajectoryTrackingImpedanceControlController] Could not find tip link name");
    return false;
  }
  if (!kdl_tree_.getChain(root_name_, tip_name_, kdl_chain_))
  {
    ROS_ERROR(
        "[TrajectoryTrackingImpedanceControlController] Could not get KDL chain from tree");
    return false;
  }
...
void TrajectoryTrackingImpedanceControlController::starting(const ros::Time& time) {
    x_d_ = KDL::Vector(x_target_start, 0.0000, 0.2500);  // Set initial desired position
    ROS_INFO("[TrajectoryTrackingImpedanceControlController] Controller started with initial x_d_.x: %f", x_d_.x());
}

void TrajectoryTrackingImpedanceControlController::update(const ros::Time& time,
                                           const ros::Duration& period)
{    
    if (trajectory_active && x_d_.x() < x_target_end) {
        x_d_.x(x_d_.x() + x_step); // Increment x component of the desired position
        if (x_d_.x() >= x_target_end) {
            trajectory_active = false; // Stop updating once the end is reached
            ROS_INFO("[TrajectoryTrackingImpedanceControlController] Target trajectory completed.");
        }
    }

    double dt = period.toSec();
    // Assuming x_d_ is updated somewhere in the code
    static KDL::Vector last_x_d = x_d_;  // Store last desired position
    KDL::Vector x_d_dot = (x_d_ - last_x_d) / dt;  // Calculate derivative
    last_x_d = x_d_;  // Update last desired position 
  ...
  // Get the current joint positions
    ...
    // Compute the error velocity (e_dot) in Cartesian space
    // e_dot is the difference between the desired velocity and actual end-effector velocity
    Eigen::VectorXd e_dot(6);
    e_dot << x_d_dot.x() - end_effector_velocity[0],
             x_d_dot.y() - end_effector_velocity[1],
             x_d_dot.z() - end_effector_velocity[2],
            -end_effector_velocity[3],  // Assuming no desired rotational velocity
            -end_effector_velocity[4],
            -end_effector_velocity[5];
...
void TrajectoryTrackingImpedanceControlController::stopping(const ros::Time& time) {}
}  // namespace advanced_robotics_control

PLUGINLIB_EXPORT_CLASS(
    advanced_robotics_control::TrajectoryTrackingImpedanceControlController,
